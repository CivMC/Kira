...
public class CreateGroupChatMessage extends RabbitMessage {
	...
	public void handle(JSONObject json, RabbitInputSupplier supplier) {
		...
		if (creator == null) {
			Kira.Companion.getInstance().getMcRabbitGateway().sendMessage(server, creatorUUID,
		...
		if (channelID == -1) {
            if (!Kira.Companion.getInstance().getKiraRoleManager().hasPermission(creator, "admin")) {
                Kira.Companion.getInstance().getMcRabbitGateway().sendMessage(server, creatorUUID,
		...
		Kira.Companion.getInstance().getMcRabbitGateway().sendMessage(server, creatorUUID, "Created channel successfully");
